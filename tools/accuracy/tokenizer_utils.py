...
class BPEStreamingDetokenizer(StreamingDetokenizer):
    ...
    def add_token(self, token):
        ...
        if not text.endswith("\ufffd") and not (
            len(v) == 1 and byte_decoder[v[0]] == 32
        ):
...
class TokenizerWrapper:
    ...
    def __init__(
        self,
        tokenizer,
        detokenizer_class: Callable[[Any], StreamingDetokenizer] = NaiveStreamingDetokenizer,
        eos_token_ids=None,
    ):
        self._tokenizer = tokenizer
...
def load_tokenizer(model_path, tokenizer_config_extra={}, eos_token_ids=None):
    ...
    if os.path.exists(tokenizer_file):
        with open(tokenizer_file, "r", encoding="utf8") as fid:
