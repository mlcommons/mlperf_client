...
def _get_begin_end_time_from_mlperf_log_detail(
    ...
    file = os.path.join(path, LOG_DETAIL_FILE)
...
def phases_check(
    ...
    def get_avg_power(power_path: str, run_path: str) -> Tuple[float, float]:
        ...
        detail_log_fname = os.path.join(run_path, LOG_DETAIL_FILE)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        "$ check.py -s <SESSION_DIRECTORY> -g <LOADGEN_LOG_FILE> -j <LOADGEN_OTHER_FILES>" \
        "$ check.py -s sample_result -g llama2_executor.log -j debug.log"
    )
    parser.add_argument("-s", "--session-directory", help="directory with session results data")
    parser.add_argument(
        "-g", "--loadgen-log-file", metavar="LOADGEN_LOG_FILE", type=str, required=True,
        help="mlperf-inference: mlperf_log_detail.txt ; mlperf-client: *_executor.log file")
    parser.add_argument(
        "-j", "--loadgen-other-files", nargs='+', metavar="LOADGEN_OTHER_FILES", type=str, required=True,
        help="List of values. mlperf-inference: mlperf_log_summary.txt ; mlperf-client: debug.log file")
    
    args = parser.parse_args()

    # error checking
    if args.loadgen_log_file is None:
        logging.error(f"-g loadgen_log_file is None")
        exit(1)
    else:
        LOG_DETAIL_FILE = args.loadgen_log_file

    if args.loadgen_other_files is None:
        logging.error(f"-j loadgen_other_files is None")
        exit(1)

    if not os.path.isdir(args.session_directory):
        logging.error(f"-s session_directory is not a directory")
        exit(1)

    for logfile in args.loadgen_other_files:
        RESULT_PATHS.append(RANGING_MODE+"/"+logfile)
        RESULT_PATHS.append(TESTING_MODE+"/"+logfile)
    print(RESULT_PATHS)
