...
cil::IHV::WindowsML::WindowsML(const API_IHV_Setup_t& api) {
  ...
  WindowsMLExecutionProviderSettings ep_settings(
...
DEFINE_IHV_CLASS_ENUMERATE_DEVICES_IMPL(cil::IHV::WindowsML)

bool cil::IHV::WindowsML::Init(const API_IHV_Init_t& api) {
  if (inference_ == nullptr) {
    api.logger(api.context, API_IHV_ERROR,
               "WindowsML->Init called before it has been created!");
    return false;
  }
...
bool cil::IHV::WindowsML::Prepare(const API_IHV_Simple_t& api) {
  if (inference_ == nullptr) {
    api.logger(api.context, API_IHV_ERROR,
               "WindowsML->Prepare called before it has been created!");
    return false;
  }
...
bool cil::IHV::WindowsML::Infer(API_IHV_Infer_t& api) {
  if (inference_ == nullptr) {
    api.logger(api.context, API_IHV_ERROR,
               "WindowsML->Infer called before it has been created!");
    return false;
  }
  ...
  if (auto llama2_inference =
    ...
    if (output.size() == 0) {
      api.logger(
          ...
          (std::string("WindowsML Run returned zero tokens! Error message: ") +
...
bool cil::IHV::WindowsML::Reset(const API_IHV_Simple_t& api) {
  if (inference_ == nullptr) {
    api.logger(api.context, API_IHV_ERROR,
               "WindowsML->Reset called before it has been created!");
    return false;
  }
...
bool cil::IHV::WindowsML::Deinit(const API_IHV_Deinit_t& api) {
  if (inference_ == nullptr) {
    api.logger(api.context, API_IHV_ERROR,
               "WindowsML->Deinit called before it has been created!");
    return false;
  }
...
DEFINE_API_IHV_BASIC_IMPL(cil::IHV::WindowsML, API_IHV_WIN_ML,
                          API_IHV_WIN_ML_VERSION)
