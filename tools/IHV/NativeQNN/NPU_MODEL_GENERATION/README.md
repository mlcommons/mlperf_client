# Optimizing AIMET Quantization workflow for Phi-4-reasoning, Phi-4-mini-instruct & Llama-3.1-8B-Instruct model for SnapDragon devices
...
## Install Notebook for Phi4 mini instruct
1.) Download Notebook Version **v1.0.0.260624** from https://qpm.qualcomm.com
...
1. Extract the QAIRT 2.46.0.260424 version sdk using 
```bash
wget https://softwarecenter.qualcomm.com/api/download/software/sdks/Qualcomm_AI_Runtime_Community/All/2.46.0.260424/v2.46.0.260424.zip
```
2. Unzip the sdk
```bash
unzip v2.46.0.260424.zip
```
...
2. Clone source model Phi-4-mini-instruct inside notebook folder using below commands.
...
1. Copy files "run_example_1_adascale.py", "run_example_1.py", "run_example_2.py", "run_example_2_sdx_elite.py", "run_example_2_agentic.py", "run_npu_bin.sh" and "requirements.txt" inside notebook folder.
...
```
model_name can be "llama3.1", "phi4" (Phi4 14B Reasoning) or "phi4mini" based on the model for which user wants to generate npu binaries.

Note: An agentic binary (with extended context lengths) is generated automatically alongside the regular binaries:
- For llama3.1, it is saved to the "llama3_agentic" folder.
- For phi4mini, it is saved to the "phi4mini_agentic" folder.
